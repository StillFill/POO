...
const routes = (
	<Grid fluid style={{ height: '100vh' }}>
		...
		<Route name="Registrar curso" path="/registrar-curso" component={RegisterClass} />
	</Grid>
);

Meteor.startup(() => {
	Tracker.autorun(() => {
		render(<Router history={createBrowserHistory()}>{routes}</Router>, document.getElementById('react-root'));
	});
});
