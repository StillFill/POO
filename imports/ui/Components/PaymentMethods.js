...
class PaymentMethods extends Component {
	...
	render() {
		const { cardNumber, ownerName, cardExpiry, cardCvc, focused, paymentSuccess, errorReason } = this.state;
		if (paymentSuccess) {
			return (
				<div>
					<h1>Pagamento efetuado com sucesso!</h1>
					<h2>
						Verifique a tela de <a onClick={() => this.pushRoute('/meus-cursos')}>meus cursos</a> para
						visualizar seus cursos!
					</h2>
				</div>
			);
		}
		return (
			<div>
				...
				<Row>
					<p style={{ width: '100%', textAlign: 'center', margin: '1rem 0', color: 'red' }}>
