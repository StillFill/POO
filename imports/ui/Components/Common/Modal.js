...
class NavBar extends Component {
	render() {
		...
		return (
			<Modal id={id} className={small ? 'small-modal' : ''} show={showModal} onHide={closeModal}>
				{title && (
					<Modal.Title>
					...
					<Modal.Footer>
						<div style={{ display: 'flex', justifyContent: 'space-between' }}>
							...
							{confirmationCallback && (
								<div>
									{disabledButton ? (
										<OverlayTrigger overlay={getTooltip(disabledReason)} placement="top">
											<button
										...
										<button className="success-button" onClick={confirmationCallback}>
											{confirmationButtonTitle}
										</button>
									)}
								</div>
